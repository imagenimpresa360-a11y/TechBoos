...
async function fixApril() {
    try {
        console.log("🧹 LIMPIANDO REGISTROS DUPLICADOS DE ABRIL...");
        await pool.query("DELETE FROM bci_income_pool WHERE fecha_banco >= '2026-04-01' AND fecha_banco <= '2026-04-30'");
        
        console.log("🚀 RE-INYECTANDO MOVIMIENTOS LIMPIOS (INGRESOS + GASTOS)...");
        ...
        for (let i = 2; i < rows.length; i++) {
            ...
            if (monto !== 0 && desc) {
                ...
                // Usamos el prefijo 'CLEAN-APR' para asegurar integridad
                const hashId = crypto.createHash('md5').update(`CLEAN-APR-${fechaStr}-${desc}-${monto}`).digest('hex');
                await pool.query(
        ...
        console.log(`✅ ABRIL REPARADO: ${count} movimientos registrados correctamente.`);
    } catch (e) {
